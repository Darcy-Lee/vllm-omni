...
Two programmatic content gates, both calibrated against a same-seed control
...
Content gates run in a dedicated verification interpreter because SheetSage2
wants a dependency set (mir_eval, pretty_midi, numpy<2) that must not leak
into the serving environment. They are enabled by three env vars:
...
Without them the test degrades to the structural byte floor, following the
MiniMax Music 3 precedent.

Weekly rather than per-PR: each request is a 32 s song and the verification
checkpoints add ~5.5 GB of downloads.
"""
...
# 400 frames of song. The byte floor sits far below the observed payload
# (~3.0 MB of 24 kHz stereo 16-bit) to leave room for an early EOS.
_FRAMES = 400
...
_CONTENT_GATES_ENABLED = all([_VERIFY_PYTHON, _CLAP_DIR, _SHEETSAGE2_DIR])
...
def test_yue2_metal_twinkle_001(omni_server, openai_client, tmp_path) -> None:
    ...
    Output Modal: audio, 24 kHz stereo
        ...
        responses = openai_client.send_audio_speech_request(
            {
                ...
                # full_model transcript gate (our content gates do the work).
    ...
    if not _CONTENT_GATES_ENABLED:
        pytest.skip("content gates need YUE2_E2E_VERIFY_PYTHON/CLAP_DIR/SHEETSAGE2_DIR")
    ...
    assert metal_prob >= 0.9, f"style gate: P(heavy metal)={metal_prob:.3f} < 0.9"

    melody_score = report["melody_score"]
    control_score = report["control_score"]
    assert melody_score >= 0.80, f"melody gate: score={melody_score:.3f} < 0.80"
